# Komorebi Documentation

*Complete documentation for the Komorebi cognitive infrastructure system.*

---

## 📚 Documentation Index

### Getting Started

| Document | Description |
|----------|-------------|
| [**GETTING_STARTED.md**](./GETTING_STARTED.md) | Installation, quick start, and basic usage |

### User Guides

| Document | Description |
|----------|-------------|
| [**API_REFERENCE.md**](./API_REFERENCE.md) | Complete REST API documentation |
| [**CONFIGURATION.md**](./CONFIGURATION.md) | Environment variables and settings |
| [**TROUBLESHOOTING.md**](./TROUBLESHOOTING.md) | Common issues and solutions |

### Developer Guides

| Document | Description |
|----------|-------------|
| [**ARCHITECTURE.md**](./ARCHITECTURE.md) | System design and technical deep dive |
| [**CONTRIBUTING.md**](./CONTRIBUTING.md) | How to contribute to the project |
| [**DEVELOPMENT_WORKFLOWS.md**](./DEVELOPMENT_WORKFLOWS.md) | AI agent collaboration guide |

### Testing

| Document | Description |
|----------|-------------|
| [**TEST_MANIFEST.md**](./TEST_MANIFEST.md) | All tests including human verification |
| [**PEDAGOGY_PLAYWRIGHT.md**](./PEDAGOGY_PLAYWRIGHT.md) | UI testing framework guide |

### Operations

| Document | Description |
|----------|-------------|
| [**DEPLOYMENT.md**](./DEPLOYMENT.md) | Production deployment guide |
| [**SECURITY.md**](./SECURITY.md) | Security considerations |
| [**CHANGELOG.md**](./CHANGELOG.md) | Version history |

### Philosophy

| Document | Description |
|----------|-------------|
| [**PEDAGOGY.md**](./PEDAGOGY.md) | Design philosophy and principles |

---

## 🚀 Quick Links

### For New Users

1. [Installation](./GETTING_STARTED.md#installation)
2. [Quick Start](./GETTING_STARTED.md#quick-start)
3. [CLI Commands](./GETTING_STARTED.md#using-the-cli)

### For Developers

1. [Architecture Overview](./ARCHITECTURE.md#system-overview)
2. [API Quick Reference](./API_REFERENCE.md#quick-reference)
3. [Contributing Guide](./CONTRIBUTING.md#getting-started)

### For Operations

1. [Docker Deployment](./DEPLOYMENT.md#docker-deployment)
2. [Production Hardening](./SECURITY.md#production-hardening)
3. [Monitoring & Logging](./DEPLOYMENT.md#monitoring--logging)

---

## 🎯 Current Status (February 2026)

### ✅ Working & Stable
- Fast chunk capture (API, Dashboard)
- Background processing & auto-summarization
- Real-time SSE updates (<5ms latency)
- Smart caching (instant page loads)
- Project organization
- Stats dashboard
- Instant tab switching (client-side filtering)

### ⚠️ Implemented but Needs LLM
- Compaction (uses simple text truncation, needs Ollama)
- Project context building (infrastructure ready)

### 🚧 Ready to Build
- MCP server integration (infrastructure exists)
- Enhanced CLI commands
- Search & full-text indexing
- Project detail views

---

## 📖 Reading Order

### New to Komorebi?

1. **[GETTING_STARTED.md](./GETTING_STARTED.md)** - Begin here (now with "What's Next" guide)
2. **[API_REFERENCE.md](./API_REFERENCE.md)** - Learn the API
3. **[CONFIGURATION.md](./CONFIGURATION.md)** - Customize settings

### Want to Contribute?

1. **[ARCHITECTURE.md](./ARCHITECTURE.md)** - Understand the system
2. **[CONTRIBUTING.md](./CONTRIBUTING.md)** - Learn the process
3. **[TEST_MANIFEST.md](./TEST_MANIFEST.md)** - Know the tests

### Deploying to Production?

1. **[DEPLOYMENT.md](./DEPLOYMENT.md)** - Deployment options
2. **[SECURITY.md](./SECURITY.md)** - Security hardening
3. **[CONFIGURATION.md](./CONFIGURATION.md)** - Production settings

---

## 📊 Documentation Stats

| Metric | Value |
|--------|-------|
| Total documents | 14 |
| Total lines | ~7,000 |
| API endpoints documented | 25+ |
| Human test procedures | 12 |
| Code examples | 100+ |

---

## 🔄 Keeping Docs Updated

When making changes to Komorebi:

1. **API changes** → Update [API_REFERENCE.md](./API_REFERENCE.md)
2. **Config changes** → Update [CONFIGURATION.md](./CONFIGURATION.md)
3. **New features** → Update [CHANGELOG.md](./CHANGELOG.md)
4. **New tests** → Update [TEST_MANIFEST.md](./TEST_MANIFEST.md)

---

*Komorebi - Capture the light filtering through the chaos.*
